...
def save_with_pandas(full_name, ID_generated, birthday):
    data = {
    'Name': [full_name],
    'ID': [ID_generated],
    'Alter': [birthday]
    ...
    df.to_csv('daten.csv', mode='a', header=False, index=False)


# Function to save the ID in a text file
def save_id_to_file(full_name, ID_generated, birthday):
    with open("/Users/luisstephan/Desktop/Python/ids.csv", "a") as file:
        file.write(f"{full_name}, {ID_generated}, {birthday}\n")

#Function to read amount of lines
def read_lines():
    with open('/Users/luisstephan/Desktop/Python/ids.csv', 'r') as file:
...
def ID_generation():
    ...
    save_with_pandas(full_name, ID_generated, birthday_and_month)
